...
def location_pipeline(mongo_uri, batch_size, sources, my_cliff, DZA_or_PAK):
    db = MongoClient(uri).ml4p
    locs = Clifftag(mongo_uri=uri, batch_size=batch_size, sources=sources, my_cliff=my_cliff, DZA_or_PAK=DZA_or_PAK)
    locs.run()
...
class Clifftag:
    ...
    def __init__(self, mongo_uri, batch_size, sources, my_cliff, DZA_or_PAK):
        """
...
if __name__ == '__main__':
    ...
    # source_domains = db.sources.distinct('source_domain', filter={'include' : True, 'primary_location' : {'$in' : ['GTM','MLI','JAM']}})
    ...
    location_pipeline(mongo_uri=uri, batch_size=128, sources=source_domains, my_cliff=my_cliff, DZA_or_PAK = False) ##DZA and PAK, set to True
